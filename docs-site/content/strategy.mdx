...
stateDiagram-v2
    ...
    PendingFill --> WaitingForReset: Entry Fill consumes reset
```
...
Entry Intent บันทึก Entry number, signal Candle, ATR และ deterministic idempotency key Input เดิมต้องสร้าง identity เดิม เพื่อให้ retry และ replay ไม่กลายเป็นการตัดสินใจใหม่
...
ATR ที่ผูกกับ signal ใช้คำนวณ Take Profit หลัง Entry Fill ซึ่งเกิดที่เปิดของ Candle ถัดไปใน Paper ดูลำดับ execution ต่อได้ที่ [Paper Trading](/paper-trading) และข้อจำกัดจำนวน Entry ที่ [Entry Pair & Cooldown](/entry-pair-cooldown)
