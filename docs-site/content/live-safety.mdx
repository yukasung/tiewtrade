...
แผนภาพแสดงว่าการยืนยันอยู่หลังผล Preflight ผู้ใช้ต้องเห็น Market Type, Mode และข้อเท็จจริงของ Binance Account ที่ตรวจแล้วก่อนเริ่ม Live ไม่มี credential value ถูกส่งไปเก็บใน application database

Preflight ตรวจ API credentials และ permissions ของ Binance Account เดียว, BTCUSDT symbol rules, balance, open orders, positions, margin mode และ leverage Spot กับ Futures ใช้ checklist ที่เหมาะกับ account facts ของตนเอง
