...
จาก tracer slice นี้จึงบังคับ one Active Bot Session และ Session ownership โดยตรงก่อนบันทึก ownership เดียวกันด้วย SQLite จากนั้นเติม public market-data runtime, Paper Futures, desktop UI และ restart Recovery ตามลำดับ dependency จนครบ flow ใช้งานจริง ไม่ถือว่าการ replay ชุดเดียวสำเร็จเท่ากับ Paper Trading Complete

Quality gate ครอบคลุม automated tests ของ business rules, persistence, Recovery และ execution boundaries รวมทั้ง fee, slippage, Funding Fee `0.00` สำหรับ Paper Futures Phase แรก, data freshness, PnL, Drawdown และ audit events
