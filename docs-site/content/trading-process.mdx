...
## From Candle to Entry Intent

Sequence แรกแสดงผู้รับผิดชอบแต่ละขั้นตั้งแต่ตรวจ Candle จน UI เห็น state ที่บันทึกแล้ว
...
## From Fill to Basket Take Profit

Sequence ถัดไปเจาะการเปลี่ยน Basket หลัง Fill และ lifecycle ของ Take Profit Order
...
ทุก decision และ side effect ที่สำคัญถูกบันทึกพร้อม Bot Session ได้แก่ signal Candle, Preset version, Entry Intent, Order, Fill, fee, slippage, funding, Basket state และ notification `session_id` เป็น ownership boundary โดยตรง และ Audit trail ต้องตอบได้ว่า Bot ตัดสินใจอะไร เมื่อใด ใช้ข้อมูลใด และ execution ให้ผลอย่างไร
